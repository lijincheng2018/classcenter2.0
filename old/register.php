...
if($_GET['action']=="post"){
    ...
    mysqli_query($conn,"INSERT INTO register_list(title,people,people_num,author,classid,bond,time) VALUES ('$title','$people','$people_num','$name','$classid','$data_list_name','$time')");
...
<div class="table-responsive">
        <table class="table table-responsive b-t b-light text-center">
          <thead><th class="text-center">登记表ID</th><th class="text-center" width="30%">登记表标题</th><th class="text-center">应完成人数</th><th class="text-center">创建人</th><th class="text-center">创建时间</th><th class="text-center">操作</th></thead>
          <tbody>
...
<?php
    for($i=0;$i<$t_num;$i++){
           ...
           if($classid==$_SESSION['classid'] || $_SESSION['usergroup']==1)
           {
               ...
               $control='<a class="btn btn-primary btn-xs" href="./showdata?id='.$id.'">查看数据</a>&nbsp;<a class="btn btn-danger btn-xs" data-toggle="modal" data-target="#myModal" onclick="check_del('.$id.')">删除</a>';
