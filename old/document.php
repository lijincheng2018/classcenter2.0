...
$result_t=mysqli_query($conn,"SELECT * FROM doc ORDER BY id asc");
$result_t_1=mysqli_query($conn,"SELECT * FROM doc ORDER BY id asc");
...
if($_GET['action']=="print"){
   include_once("xlsxwriter.class.php");
    ...
    $tmp=$_SESSION['username'].'-个人履历'.date("YmdHis",time()).$rand_num.'.xlsx';
    $filename = 'public/download/'.$tmp;
    
    $rows = array(
        array('序号','记录名称','时长','地点','记录类型','时间','学号','登记人'),
    );
    ...
    for($i=1;$i<=$t_num;$i++){
        ...
        if($classid==$_SESSION['classid']){
            ...
            $r_time=$result_arr['r_time'];
            $place=$result_arr['place'];
            $time=$result_arr['time'];
            $dengji=$result_arr['dengji'];
            $classid=$result_arr['classid'];
            $author=$result_arr['author'];
            
            if($dengji=="1") $dengji="志愿活动";
            else if($dengji=="2") $dengji="社会实践";
            else if($dengji=="3") $dengji="参与比赛";
            else if($dengji=="4") $dengji="研究发明";
            else if($dengji=="5") $dengji="软件著作";
            else if($dengji=="6") $dengji="其他";
...
if($_GET['action']=="post"){
    ...
    mysqli_query($conn,"INSERT INTO doc(title,r_time,dengji,time,place,classid,author) VALUES ('$title','$r_time','$dengji','$time','$place','$classid','$name')");
...
if($_GET['action']=="del"){
    ...
    if($id!=""){
        mysqli_query($conn,"DELETE FROM doc WHERE id = $id");
        exit('success');
    }
    else exit('error');
}
...
if($_GET['action']=="query"){
    ...
    $result=mysqli_query($conn,"SELECT * FROM doc WHERE id='$id'");
    $row=mysqli_fetch_assoc($result);
    ...
    exit($title.'+ljc+'.$r_time.'+ljc+'.$place.'+ljc+'.$dengji.'+ljc+'.$time.'+ljc+'.$author);
}
...
?>
  ...
  <!-- / aside -->
      ...
      <div class="wrapper" id="post_activity" style="display:block">
        <div class="col-sm-12">
            <a class="btn btn-success" href="javascript:;" onclick="post()" id="post_activity">新增记录</a>
        </div>
      </div>
...
<div class="panel-heading font-bold" style="background-color: #9999CC;color: white;" >新增履历</div>
<div class="panel-body">
  <form name="RegForm" id="RegForm" enctype="multipart/form-data" action="#" method="post" role="form">
	...
	<div class="form-group">
	  <label>时长:（单位：小时）</label><br/>
	  <input type="text" name="r_time" id="r_time" value="" class="form-control"/>
	</div>
	<div class="form-group">
	  <label>地点:</label><br/>
	...
	<div class="form-group">
	  ...
	  <select id="dengji" class="selectpicker show-tick form-control">
			<option value="1">志愿活动</option>
			<option value="2">社会实践</option>
			<option value="3">参与比赛</option>
			<option value="4">研究发明</option>
			<option value="5">软件著作</option>
...
<div class="table-responsive">
        <table class="table table-responsive b-t b-light text-center">
          <thead><th class="text-center">序号</th><th class="text-center" width="20%">记录名称</th><th class="text-center" width="20%">时长</th><th class="text-center">地点</th><th class="text-center" width="20%">记录类型</th><th class="text-center">时间</th><th class="text-center">操作</th></thead>
          <tbody>
...
<?php
        ...
        for($i=0;$i<$t_num;$i++){
            ...
            if($classid==$_SESSION['classid'])
            {
               ...
               $control='<a class="btn btn-primary btn-xs"  data-toggle="modal" data-target="#myModal" onclick="query('.$id.')">查看详情</a>&nbsp;<a class="btn btn-success btn-xs" href="editdoc?id='.$id.'">编辑</a>&nbsp;<a class="btn btn-danger btn-xs" href="javascript:;" onclick="del('.$id.')">删除</a>';
			...
			<h4 class="modal-title" id="myModalLabel">
              	详细资料
						...
						<div class="modal-body" style="text-align:left">
							记录名称：<span id="s_title"></span><br>
							时长：<span id="s_r_time"></span>小时<br>
							地点：<span id="s_place"></span><br>
							记录类型：<span id="s_dengji"></span><br>
							时间：<span id="s_time"></span><br>
...
<script language="JavaScript">
    function query(getid){
        $.get('?action=query&id='+getid,function(data){
                ...
                strs=data.split("+ljc+");
                $('#s_title').text(strs[0])
                $('#s_r_time').text(strs[1])
                $('#s_place').text(strs[2])
                $('#s_dengji').text(strs[3])
                $('#s_time').text(strs[4])
