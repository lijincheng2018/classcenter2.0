...
<div id="content" role="main">
    		...
    		if($_FILES["file1"]["size"]<51200000)
            {
            	if ($_FILES["file1"]["error"] > 0){
                    ...
                    if($reg_ifrename=="yes"){
                        ...
                        $url2 = $url1.$confname1.'/';
                        if(!is_dir($url2)) mkdir($url2,0755,true);
                        
                        $upload_file_name=$confname1.'.'. $ext1;
                        $path1 = move_uploaded_file($_FILES["file1"]["tmp_name"], $url2 . $confname1 .'.'. $ext1);
                    }
                    else{
                        $whconf1 = $url2.$_FILES['file1']['name'];
  ...
  ?>
  <?php if($form_id=="20011" || $form_id=="20010") echo '<div class="col-sm-12" id="ljc_bg">';
  else echo '<div class="col-sm-12 col-md-9" id="ljc_bg">';?>
			...
			<div class="panel panel-default">
				...
				<div style="background:#fff" class="panel-body text-center">
					...
					<form action="?id=<?=$form_id?>&action=upload" method="post" class="form-horizontal" id="RegForm" name="RegForm" role="form" enctype="multipart/form-data" onsubmit="return InputCheck(this)">
						...
						<p style="text-align:center;color:red;">允许提交的文件后缀：
							<?php echo substr($format_list,0,strlen($format_list)-1);?>
						</p>
				...
				<div class="panel-footer">
					<span class="glyphicon glyphicon-info-sign"></span>&nbsp;发布者：
					<?=$reg_author?>
				</div>
			</div>
		</div>
<?php if($form_id=="20011" || $form_id=="20010") echo '<div class="col-sm-12 col-md-3" style="display:none">';
  else echo '<div class="col-sm-12 col-md-3">';?>
			...
			<div class="panel panel-success text-center" id="recharge">
				...
				<div class="panel-body">
					<div class="table-responsive">
						<table class="table" style="white-space:nowrap;padding:0px;">
							<tbody>
								<?php
					...
					<div class="panel-footer" style="padding:0px">排行榜仅展示前十</div>
				</div>
			</div>
		</div>
	</div>
